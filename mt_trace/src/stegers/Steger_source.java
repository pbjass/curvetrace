...
//	String str= "/Users/winja/Desktop/SIV_WORK/CurveTrace1/mt_trace/data/Arya.png" ;
	...
	public static double angledev = 20;   
	//int nRoiColor;
		 ...
		 double langle;
		  ...
		  for(int i=0;i<pointtmcont.size();i++) {
				...
				// System.out.println(px1 +", "+ px2);
					 for(int j=i+1;j<pointtmcont.size();j++) {
...
//					    	b=pxs_al.get(k).floatValue();
			  ...
			  // split the contours and save it in a new pointcontour(no of p)
			   ...
			   tmpcont=pointtmcont.get((int)46);
			   num_pnt=tmpcont.num; 
			   double [] p0 =new double[2]; double []p1 =new double[2];
			  double []p2 =new double[2];
					double pangle;
			   for(int j1=0;j1<tmpcont.num && (j1+1)<tmpcont.num &&(j1+2)<tmpcont.num; j1++) {
//		  			double [] p0 ,p1,p2;
//		  			double pangle;
		  			p1[0]= (tmpcont.col.get((int)j1)).doubleValue();
		  			p1[1]= (tmpcont.row.get((int)j1)).doubleValue();
		  			
		  			p0[0]= (tmpcont.col.get((int)(j1+1))).doubleValue();
		  			p0[1]=( tmpcont.row.get((int)(j1+1))).doubleValue();
		  			
		  			p2[0]= (tmpcont.col.get((int)(j1+2))).doubleValue();
		  			p2[1]= (tmpcont.row.get((int)(j1+2))).doubleValue();
		  			
		  			
		  			pangle= computeAngle(p0,p1,p2);
		  			System.out.println(Math.toDegrees(pangle)+" ");
			   }
			 //  blockcombine();
			  //new point contour after split
...
//				  Collections.reverse(tmcont.row);
//				  Collections.reverse(tmcont.col);
//				  Collections.reverse(tmcont.width_l);
//				  Collections.reverse(tmcont.width_r);
